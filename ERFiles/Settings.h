...
public:
	...
	enum ReplacementType {
		...
		REPLACEMENT_REMOVEWORST = 1,
		REPLACEMENT_PARETOMORPH = 2,
		REPLACEMENT_GENERATIONAL = 3
	};
...
//		NEAT_CONTROL = 5
	...
	enum InstanceType {
		...
		INSTANCE_PHYSICAL = 3 // physical should read from queue
	};
	...
	SelectionType selectionType = SELECTION_RANDOM;
	ReplacementType replacementType = REPLACEMENT_REMOVEWORST;
...
//	EvolutionType evolutionType = GENERATIONAL; // not implemented yet
  ...
  int initialPopulationSize = 100;
	...
	vector<int> coIndNumbers;
	vector<float> indFits;
	vector<float> coIndFits;
	int seed = 0;
	// modular parameters
	int amountModules = 5;
	int useVarModules = 0;
	float maxForce = 1.5;
	float maxForceSensor = 0.1f; // N*m
	float maxTorqueSensor = 0.1f;
	int consecutiveThresholdViolations = 2;
