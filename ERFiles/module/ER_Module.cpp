...
void ER_Module::colorModule(float color[3], float alpha) {
	...
	if (settings->colorization == settings->COLOR_NEURALNETWORK) {
		int alp = (int)(1.0 * alpha * 255.0);
		double alphaCol[3] = { _cm[alp][0], _cm[alp][1], _cm[alp][2] };
		newColor[0] = alphaCol[0] * 0.5;
		newColor[1] = alphaCol[1] * 0.5;
		newColor[2] = alphaCol[2] * 0.5;
	}
	for (int i = 0; i < 3; i++) {
	...
	for (int it = 0; it < values.size(); it++) {
		...
		if (tmp == "#ControlType") {
			...
			if (controlType != -1) {
				...
				control = controlFactory->createNewControlGenome(controlType, settings, randomNum); // 0 is ANN, 1 is Custom_ANN
