...
{
	if (settings->SELECTION_RANDOM) {
		createNewGenRandomSelect();
	}
	else if (settings->SELECTION_PROPORTIONATE) {
		createNewGenProportionate();
	}
}
...
void EA_SteadyState::createNewGenProportionate() {
	nextGenGenomes.clear();
	//	nextGenFitness.clear();
	shared_ptr<MorphologyFactory> mfact(new MorphologyFactory);
	for (int i = 0; i < populationGenomes.size(); i++) {
	...
	for (int p = 0; p < populationGenomes.size(); p++) {
		for (int n = 0; n < numAttempts; n++) {
			...
			if (nextGenGenomes[p]->fitness >= populationGenomes[currentInd]->fitness) {
				...
				// delete genome file
				stringstream ss;
				ss << settings->repository + "/genomes" << settings->sceneNum << "/genome" << nextGenGenomes[p]->individualNumber << ".csv";
				string genomeFileName = ss.str();
				stringstream ssp;
				ssp << settings->repository + "/genomes" << settings->sceneNum << "/phenotype" << nextGenGenomes[p]->individualNumber << ".csv";
