...
{
	shared_ptr<DefaultGenome> g = make_unique<DefaultGenome>(*this);
	g->morph = morph->clone();
	return g;
}
...
bool DefaultGenome::loadMorphologyGenome(int indNum, int sceneNum) {
	...
	morph = morphologyFactory->createMorphologyGenome(m_type);
...
void DefaultGenome::init_noMorph() {
	...
	//	control = controlFactory->createNewControlGenome(0, settings, randomNum);
...
void DefaultGenome::init() {
	...
	if (settings->verbose)
	{
		cout << "morphology type = " << settings->morphologyType << endl;
	}
	shared_ptr<MorphologyFactory> morphologyFactory(new MorphologyFactory);
	morph = morphologyFactory->createMorphologyGenome(m_type);
	morphologyFactory.reset();
...
//		unique_ptr<ControlFactory> controlFactory(new ControlFactory);
//		morph->control = controlFactory->createNewControlGenome(0, settings, randomNum); // ann
//		controlFactory.reset();
//	}
}
