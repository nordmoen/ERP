...
{ // This is called just once, at the start of V-REP.
	...
	if (startEvolution == true) {
		...
		ER->settings = Settings::getInstance();
...
//		}
		...
		if (arg2_param != NULL) {
			...
			if (str.length() > 0) {
				...
				else if (arg2_param_i == 11) {
					ER->simSet = RECALLINDIVIDUAL;
					// TODO
					ER->settings->instanceType = ER->settings->INSTANCE_REGULAR;
					ER->settings->morphologyType = ER->settings->MODULAR_PHENOTYPE;
					simChar* arg4_param = simGetStringParameter(sim_stringparam_app_arg4);
					const int arg4_param_i = atoi(arg4_param);
					// This should be the individual that we want to load
					simReleaseBuffer(arg4_param);
				}
		...
		ER->randNum = RandNum::getInstance();
		ER->randNum->setSeed(run);
