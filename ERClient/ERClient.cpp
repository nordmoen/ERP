...
int main(int argc, char* argv[])
{
	...
	int amountGen = 0;
	client->settings = Settings::getInstance();
	...
	if (arguments.size() > 1) {
		...
		client->randNum = RandNum::getInstance();
	...
	extApi_sleepMs(5000); // wait 20 seconds before connecting to ports, gives v-rep time to start up.
